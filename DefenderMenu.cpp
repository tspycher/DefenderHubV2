...
DefenderMenu::DefenderMenu(Adafruit_SSD1351 &oled, Defender &car)
  : oled(oled), car(car), num_pages(0), current_page_index(0), interrupt_switch_page(false), status_indicator_shown(false) {
}
...
void DefenderMenu::begin() {
  ...
  pages[num_pages] = new PagePdc(oled, car);
  Serial.println("****** initialized PDC Page");
  ++num_pages;
